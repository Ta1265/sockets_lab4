...
//host server_socket_2 port on 9996
    ...
    char server1_message[256] = "Hello message from cleint 5";
    send(client_socket, server1_message, sizeof(server1_message),0);

//connect to client 6
//connect to server_socket_2 on port 9995
    int client_socket_2;
    ...
    server_address_3.sin_port = htons(9995); //port
    ...
    if(connect_status == -1){
        printf("connection error code on client 5: %d \n", connect_status);
    }

    char server_response[256];
    recv(client_socket_2, &server_response, sizeof(server_response), 0);
    printf("Received on client 5 Server response: %s \n", server_response);
