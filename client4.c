...
//host server_socket_2 port on 9997
    ...
    char server1_message[256] = "Hello message from cleint 4";
    send(client_socket, server1_message, sizeof(server1_message),0);

//connect to client 5
//connect to server_socket_2 on port 9996
    int client_socket_2;
    ...
    server_address_3.sin_port = htons(9996); //port
    ...
    if(connect_status == -1){
        printf("connection error code on client 4: %d \n", connect_status);
    }

    char server_response[256];
    recv(client_socket_2, &server_response, sizeof(server_response), 0);
    printf("Received on client 4 Server response: %s \n", server_response);
